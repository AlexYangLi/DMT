...
def load_trained(load_filename, vocabulary):
    ...
    print('Loggong Info - From %s Embedding matrix created : %s, unknown tokens: %s', load_filename, emb.shape, nb_unk)
    return emb
...
def train_w2v(corpus, cut_func, vocabulary, embedding_dim=300):
    ...
    for w, i in vocabulary.items():
        if w not in d:
            ...
            emb[i, :] = weights[d[w], :]
    print('Logging Info - Word2Vec Embedding matrix created: %s, unknown tokens: %s', emb.shape, nb_unk)
    return emb
